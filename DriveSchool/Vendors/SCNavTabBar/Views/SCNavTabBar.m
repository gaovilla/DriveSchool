...
//    _line = [[UIView alloc] initWithFrame:CGRectMake(2.0f, NAV_TAB_BAR_HEIGHT - 3.0f, width - 4.0f, 3.0f)];
    _line = [[UIView alloc] initWithFrame:CGRectMake(2.0f, NAV_TAB_BAR_HEIGHT - 3.0f, SCREEN_WIDTH/[_items count] - 4.0f, 3.0f)];
...
//    [self showLineWithButtonWidth:[widths[0] floatValue]];
    ...
    {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(buttonX, DOT_COORDINATE, SCREEN_WIDTH/[_itemTitles count], NAV_TAB_BAR_HEIGHT);
        [button setTitle:_itemTitles[index] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [button addTarget:self action:@selector(itemPressed:) forControlEvents:UIControlEventTouchUpInside];
        [_navgationTabBar addSubview:button];
        ...
        [_items addObject:button];
        buttonX += SCREEN_WIDTH/[_itemTitles count];
    }
...
//    }];
    //zxc
    [UIView animateWithDuration:0.2f animations:^{
        _line.frame = CGRectMake(button.frame.origin.x + 2.0f, _line.frame.origin.y, SCREEN_WIDTH/[_itemsWidth count] - 4.0f, _line.frame.size.height);
    }];
}
